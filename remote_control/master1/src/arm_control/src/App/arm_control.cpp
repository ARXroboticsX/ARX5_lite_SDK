    ...
    else
    {
        if(control_mode == 0  ||  control_mode == 1)
        {
                ...
                if(teach2pos_returning ){
                    ...
                    is_starting=0;
                    temp_init=0;
                    CAN_Handlej.control(target_pos,motor_control_cmd,is_teach_mode,current_normal,solve.joint_torque,ecat_base);
                }
...
void arx_arm::init_step(ecat::EcatBase ecat_base)
{
   ...
   if(temp_init>2)
   {
            ...
            CAN_Handlej.Send_moto_Cmd2(ecat_base,1, kp_40, 1, target_pos[0], 0,  0); usleep(200);         
            CAN_Handlej.Send_moto_Cmd2(ecat_base,2, kp_40, 1, target_pos[1], 0,  0); usleep(200);
            CAN_Handlej.Send_moto_Cmd2(ecat_base,4, kp_40, 1, target_pos[2], 0,  0); usleep(200);
